...
class TestEndpoints:
    ...
    def test_chat_success(self, mock_gemini_cls, mock_vertex_cls, client):
        ...
        resp = client.post(
            "/api/chat", json={"message": "hello", "session_id": "s1"}
        )
        assert resp.status_code == 200
    ...
    def test_chat_validation_errors(self, client):
        ...
        resp = client.post(
            "/api/chat", json={"message": "  <script></script>  "}
        )
        assert resp.status_code == 400
    ...
    def test_translate_endpoint(self, mock_trans_cls, client):
        ...
        mock_trans.translate_text.return_value = {
            "success": True,
        ...
        resp = client.post(
            "/api/translate", json={"text": "hello", "target_language": "es"}
        )
        assert resp.status_code == 200
    ...
    def test_translate_languages(self, mock_trans_cls, client):
        mock_trans = MagicMock()
        mock_trans.get_supported_languages.return_value = {
    ...
    def test_detect_language(self, mock_trans_cls, client):
        mock_trans = MagicMock()
        mock_trans.detect_language.return_value = {
            "language": "en",
            "success": True,
        }
        mock_trans_cls.return_value = mock_trans
    ...
    def test_session_creation(self, mock_fb_cls, client):
        mock_fb = MagicMock()
        mock_fb.create_session.return_value = {
            "session_id": "s1",
            "success": True,
        }
    ...
    def test_quiz_score(self, mock_fb_cls, client):
        ...
        resp = client.post(
            "/api/session/s1/quiz", json={"score": 5, "total": 10}
        )
        assert resp.status_code == 200
    ...
    def test_413_error(self, client):
        ...
        resp = client.post(
            "/api/chat", data="x" * 15000, content_type="application/json"
        )
    ...
    def test_persist_chat_exception_swallowed(self, mock_persist, app):
        ...
        with patch(
            "services.firebase_service.FirebaseService.get_instance",
            side_effect=Exception("FB fail"),
        ):
    ...
    def test_detect_language_route(self, mock_svc_cls, client):
        mock_svc = MagicMock()
        mock_svc.detect_language.return_value = {
            "language": "en",
            "success": True,
        }
    ...
    def test_save_quiz_score_route(self, mock_svc_cls, client):
        ...
        resp = client.post(
            "/api/session/s1/quiz",
            json={"score": 5, "total": 10, "topic": "voting"},
        )
        assert resp.status_code == 200
    ...
    def test_server_error_handler(self, client):
        ...
        with client.application.test_request_context():
            resp = client.application.handle_user_exception(
                InternalServerError()
            )
